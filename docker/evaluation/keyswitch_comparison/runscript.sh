#!/bin/bash
# Run keyswitch comparison simulations and generate keyswitch_comparison.pdf
set -eou pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUTPUT_DIR=/cinnamon_artifact/outputs
RESULTS_DIR=${OUTPUT_DIR}/keyswitch_results
...
run_sim() {
    ...
    ${SST_BIN} "${SCRIPT_DIR}/cinnamon_config.py" -- \
        ...
        --trace_dir "${TRACE_DIR}/keyswitch_${num_chiplets}chiplet" \
        ${extra_args} \
        2>&1 | tee "${log}"
    echo "Done:    ${label}"
}
...
# Generate figure
# ---------------------------------------------------------------------------
echo "Plotting keyswitch comparison ..."
python3 "${SCRIPT_DIR}/plot.py" \
    --results_dir "${RESULTS_DIR}" \
    --output      "${OUTPUT_DIR}/keyswitch_comparison.pdf"

echo "keyswitch_comparison.pdf written to ${OUTPUT_DIR}"
